...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<{
    ...
    const activeMembership = await this.resolveActiveMembership(user.id);
    const tokens = await this.issueTokenPair(user, activeMembership?.tenantId ?? null);
  ...
  async loginWithGoogle(profile: GoogleOAuthProfile): Promise<{
    ...
    const activeMembership = await this.resolveActiveMembership(persistedUser.id);
  ...
  private toAuditActor(user: UserEntity): AuthenticatedUser {
    return {
      ...
      email: user.email,
      role: user.role,
      permissions: this.toPermissionCodes(
  ...
  async refreshToken(refreshTokenDto: RefreshTokenDto): Promise<{
    ...
    const preferredTenantId = refreshTokenRecord.tenantId ?? null;
    const activeMembership = await this.resolveActiveMembership(
      user.id,
      preferredTenantId,
    );
    const tokens = await this.issueTokenPair(user, activeMembership?.tenantId ?? null);
  ...
  async switchTenant(
    ...
    const membership = await this.resolveActiveMembership(user.id, tenantId);
    ...
    const tokens = await this.issueTokenPair(foundUser, tenantId);
    return {
      access_token: tokens.accessToken,
      refresh_token: tokens.refreshToken,
    };
  ...
  private async issueTokenPair(
    ...
    const activeMembership = activeTenantId
      ? await this.resolveActiveMembership(user.id, activeTenantId)
      : await this.resolveActiveMembership(user.id);
    const resolvedTenantId = activeMembership?.tenantId ?? null;
    ...
    const accessPayload: AccessTokenPayload = {
      ...
      platform_permissions: this.extractPlatformPermissions(
        user.permissions?.map((permission) => String(permission.code)) ?? [],
      ),
      active_tenant_id: resolvedTenantId,
      role_in_tenant: activeMembership?.role ?? null,
  ...
  private async resolveActiveMembership(
    ...
    const activeMemberships = await this.membershipsRepository.find({
      where: {
        userId,
    ...
    const byPriority = [...activeMemberships].sort((left, right) => {
      const scoreLeft = this.membershipRolePriority(left.role);
      const scoreRight = this.membershipRolePriority(right.role);
  ...
  private membershipRolePriority(role: TenantMembershipRole): number {
    switch (role) {
  ...
  private toPermissionCodes(codes: string[]): PermissionCode[] {
    return codes.filter((code): code is PermissionCode =>
      Object.values(PermissionCode).includes(code as PermissionCode),
    );
  }
}
