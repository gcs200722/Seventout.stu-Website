...
export class NotificationService {
  ...
  async listNotifications(
    ...
    const qb = this.notificationsRepository
      ...
      .andWhere('notification.user_id = :userId', { userId: user.id })
  ...
  async processEmailJob(payload: {
    ...
    const notification = await this.notificationsRepository.findOne({
      where: { id: payload.notification_id, tenantId: payload.tenant_id },
    });
    if (!notification) {
      return;
    }
  ...
  private async dispatchTemplate(
    ...
    if (
      ...
      await this.queuePort.enqueue(
        'send_email',
        {
          tenant_id: systemNotification.tenantId,
  ...
  private isDuplicateDedupeError(error: unknown): boolean {
    ...
    return (
      ...
      (driverError?.constraint === 'uq_notifications_dedupe' ||
        driverError?.constraint === 'uq_notifications_tenant_dedupe')
    );
  }
  ...
  private async resolveTenantId(): Promise<string> {
    ...
    if (Array.isArray(rows) && rows.length > 0) {
      const first = rows[0] as { id?: unknown };
      if (typeof first.id === 'string') {
        this.defaultTenantId = first.id;
        return this.defaultTenantId;
      }
    }
    throw new ForbiddenException('Default tenant is not configured.');
  }
}
