...
export class CartService {
  ...
  async getCurrentCart(userId: string): Promise<CartSnapshot> {
    ...
    const cached = await this.cartCache.get(cacheKey);
    if (cached) {
      return cached;
    }
    const cart = await this.ensureActiveCart(tenantId, userId);
    const payload = await this.buildSnapshot(tenantId, cart.id);
    await this.cartCache.set(cacheKey, payload);
    return payload;
  }
  ...
  async addItem(userId: string, payload: AddCartItemDto): Promise<void> {
    const tenantId = this.tenantContext.requireTenantId();
    const cart = await this.ensureActiveCart(tenantId, userId);
    const product = await this.getActiveProduct(tenantId, payload.product_id);
    const variant = await this.getVariantForProduct(
      payload.product_variant_id,
      product.id,
      tenantId,
    );
    const stock = await this.getAvailableStock(variant.id, tenantId);
    ...
    await this.cartCache.invalidate(this.cacheKey(tenantId, userId));
  }
  ...
  async updateItem(
    ...
    const product = await this.getActiveProduct(tenantId, item.productId);
    const targetVariantId =
      payload.product_variant_id && payload.product_variant_id.trim().length > 0
        ? payload.product_variant_id.trim()
        : item.productVariantId;
    await this.getVariantForProduct(targetVariantId, product.id, tenantId);
    ...
    const requiredQuantity = shouldMerge
      ...
      : payload.quantity;
    const stock = await this.getAvailableStock(targetVariantId, tenantId);
  ...
  async validateCart(userId: string): Promise<ValidateCartResult> {
    const tenantId = this.tenantContext.requireTenantId();
    const cart = await this.ensureActiveCart(tenantId, userId);
    return this.validateCartById(tenantId, cart.id);
  }
  ...
  private async ensureActiveCart(
    ...
    let cart = await this.cartsRepository.findOne({
      where: { tenantId, userId, status: CartStatus.ACTIVE },
    });
    if (!cart) {
  ...
  private async validateCartById(
    tenantId: string,
    cartId: string,
    ...
    const variants = await this.variantsRepository.find({
      where: { tenantId, id: In(items.map((item) => item.productVariantId)) },
    });
