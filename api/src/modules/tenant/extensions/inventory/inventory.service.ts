...
export class InventoryService {
  ...
  async listInventory(query: ListInventoryQueryDto): Promise<{
    ...
    const key = JSON.stringify({ tenant_id: tenantId, ...query });
    ...
    const rows =
      variantIds.length === 0
        ...
        : await this.inventoriesRepository.find({
            where: { productVariantId: In(variantIds), tenantId },
            order: { channel: 'ASC' },
          });
  ...
  async listMovements(query: ListInventoryMovementsQueryDto): Promise<{
    ...
    qb.where('movement.tenant_id = :tenantId', { tenantId });
    if (query.product_variant_id) {
  ...
  private async ensureProductExists(productId: string): Promise<void> {
    const product = await this.productsRepository.findOne({
      where: { id: productId, tenantId: await this.resolveTenantId() },
    });
  ...
  private async ensureVariantExists(
    ...
    const variant = await this.variantsRepository.findOne({
      where: { id: productVariantId, tenantId: await this.resolveTenantId() },
