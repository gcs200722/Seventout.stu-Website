...
import { DEFAULT_QUEUE_NAME } from '../../core/queue/queue.constants';
...
export class InventorySyncProcessor extends WorkerHost {
  ...
  private async processSyncStock(
    ...
    const tenantId = this.readStringField(job.data, 'tenant_id');
    ...
    const mapping = await this.mappingsRepository.findOne({
      where: { tenantId, productVariantId, channel, isActive: true },
    });
    if (!mapping) {
  ...
  private async processWebhookEvent(
    ...
    await this.inventoryWebhookService.markWebhookProcessed(
      tenantId,
      channel,
      eventId,
    );
