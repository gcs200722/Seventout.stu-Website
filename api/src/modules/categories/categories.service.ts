...
export class CategoriesService {
  ...
  async listCategories(
    ...
    const where: FindOptionsWhere<CategoryEntity> = { deletedAt: IsNull() };
  ...
  async createCategory(
    ...
    await this.ensureUniqueName(payload.name, parent?.id ?? null);
  ...
  async softDeleteCategory(
    ...
    const childrenCount = await this.categoriesRepository.count({
      where: { parentId: category.id, deletedAt: IsNull() },
    });
    if (childrenCount > 0) {
    ...
    const isUsedByProduct = await this.isCategoryUsedByProduct(category.id);
    if (isUsedByProduct) {
      throw new BadRequestException({
  ...
  private async isCategoryUsedByProduct(categoryId: string): Promise<boolean> {
